...
create table compte (
             no number(7), type_compte_no number(7), dateOuverture date, decouvert_autorise number(7,2), 
             solde number(11,2), client_no number(7), agence_no number(7),
             Constraint compte_pk primary key (no), 
			 constraint type_compte_fk foreign key (type_compte_no) references type_compte(no), 
...
create table type_operation (
                        no number(7), libelle_operation varchar2(40), description varchar2(100),
                        Constraint type_operation_pk primary key (no)
						);


create table operation (
                        no number(7), type_operation_no number(7), compte_no number(7),
